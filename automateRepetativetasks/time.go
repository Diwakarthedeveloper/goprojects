...
func main() {
	...
	ReadFile("C:/yyy/xxx/data/example.txt")
	// Write to a log file
	WriteLog("C:/yyy/xxx/data/activity.log", "Routine task executed.")
	...
	go func() {
		for {
			...
			case <-ticker.C:
				// Perform the task
				WriteLog("C:/yyy/xxx/data/activity.log", "Scheduled task executed.")
			case <-quit:
				ticker.Stop()
				return
			}
